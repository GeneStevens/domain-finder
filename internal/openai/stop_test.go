...
func TestStopControllerStopsOnCostCap(t *testing.T) {
	...
	stop := controller.ObserveBatch(2, 0, 0, &Usage{InputTokens: 120, OutputTokens: 18, CachedInputTokens: 40})
	if stop == nil || stop.Reason != StopReasonCostCapReached {
		t.Fatalf("ObserveBatch() = %#v, want cost-cap stop", stop)
	}
}

func TestStopControllerStopsOnAvailableHitTarget(t *testing.T) {
	controller, err := NewStopController("gpt-4o-mini", StopConditions{
		MaxAccepted:         10,
		TargetAvailableHits: 3,
	}, 1, 0)
	if err != nil {
		t.Fatalf("NewStopController() error = %v", err)
	}
	stop := controller.ObserveBatch(2, 2, 0, nil)
	if stop == nil || stop.Reason != StopReasonAvailableHitsReached {
		t.Fatalf("ObserveBatch() = %#v, want available-hit stop", stop)
	}
}
...
func TestStopControllerStopsOnStrongHitTarget(t *testing.T) {
	...
	stop := controller.ObserveBatch(2, 2, 2, nil)
...
func TestStopControllerStopsOnStallLimit(t *testing.T) {
	...
	if stop := controller.ObserveBatch(0, 0, 0, nil); stop != nil {
		t.Fatalf("first stall = %#v, want nil", stop)
	}
	stop := controller.ObserveBatch(0, 0, 0, nil)
...
func TestNewStopControllerRejectsUnknownPricingForCostCap(t *testing.T) {
	if _, err := NewStopController("custom-model", StopConditions{MaxAccepted: 10, MaxCostUSD: 1.0}, 0, 0); err == nil {
