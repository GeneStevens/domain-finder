...
func NewPromptInput(theme string, generate config.GenerateConfig) PromptInput {
	return PromptInput{
		...
		TargetAvailableHits: generate.TargetAvailableHits,
...
func (b PromptBuilder) BuildContract(cfg config.Config, theme string) Contract {
	...
	return Contract{
		...
		TargetAvailableHits: input.TargetAvailableHits,
...
func (PromptBuilder) BuildUserPrompt(input PromptInput, count int) string {
	...
	if input.TargetAvailableHits > 0 {
		lines = append(lines, fmt.Sprintf("Run policy: generation may stop once %d available hits are found in at least one requested zone.", input.TargetAvailableHits))
	}
...
func RenderContract(contract Contract) string {
	...
	fmt.Fprintf(&out, "  target_available_hits: %s\n", renderOptionalInt(contract.TargetAvailableHits))
...
func RenderContractJSON(contract Contract) ([]byte, error) {
	...
	payload := view{
		...
		Constraints: constraints{
			...
			TargetAvailableHits: contract.TargetAvailableHits,
